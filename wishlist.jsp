		...
		<script>
  		...
  		<script>
  		  function deleteItem(itemId){	
  				...
  				$.ajax({				
	                type: "post",
	                url: "/Zhen2b/UnLikeServlet",     
	                data:{likeditemid:itemId}	                										
	          	});
  				window.setInterval(function(){location.href='/Zhen2b/wishlist.jsp'}, 1500);
  		  ...
  		  function setTimer(a){	
  			  ...
  			  var itemId=a.id;
  			  //alert(itemId);
  			  var timeSet=document.getElementById(a.name).value;	
  			  //alert(timeSet);
  			
  			  $.get("TimerServlet",{itemId:itemId,timeSet:timeSet},function(){});
			  alert("Timer has been added !");
			  window.setInterval(function(){location.href='/Zhen2b/wishlist.jsp'}, 1500);
  		}
  		</script>
		<style type="text/css">
								...
								<div class="tab-pane active" id="step1">
									<div class="row-fluid">
										...
										<script>
										 $.ajax({				
							                type: "post",
							                url: "/Zhen2b/WishListServlet",
							                dataType: "json",
							                success: postWishList, error: function(){alert("Wrong,Login!");}										
							          	}); 
										function postWishList(data){												
											...
											if(data.length==0){
												...
												for(var i=0;i<data.length;i++){
													...
													html.push('<td><input class="input-micro" id="'+i+'" name="timeSet" value="1" type="text">'+
															'<button type="button" class="btn pull-right" id="'+itemId+'" name="'+i+'" onClick="setTimer(this)">Set</button></td>');
									...
									<div class="row-fluid">
										<div class="span4">
											<button class="btn" onClick="location.href='indexAfterLogin.jsp'"><i class="icon-chevron-left"></i> Continue shopping</button>
										</div>
										<div class="span4">
										</div>
										<div class="span4">
											<button class="btn btn-primary pull-right" onClick="javascript:location.href='timer.jsp'">View Timers <i class="icon-chevron-right icon-white"></i></button>
										</div>
									</div>
								</div>
